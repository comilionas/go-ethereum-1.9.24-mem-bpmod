...
func (api *PublicFilterAPI) NewPendingTransactionFullFilter() rpc.ID {
	...
	api.filtersMu.Lock()
	api.filters[pendingTxSub.ID] = &filter{typ: PendingTransactionsFullSubscription, deadline: time.NewTimer(deadline), transactions: make([]*types.TxDetails, 0), s: pendingTxSub}
	api.filtersMu.Unlock()
